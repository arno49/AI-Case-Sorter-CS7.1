...
tests, 116 host tests, 299 daemon/simulator tests, 508 appliance web tests,
and 30 appliance ops artifact tests. Unchecked criteria below
...
- [x] PI-DAEMON-001: implement the sole serial worker, bounded normal admission,
      coalesced priority-stop lane, and single-open/concurrency evidence.
- [x] PI-DAEMON-002: publish session states and conservative reconnect without
      command replay. Real POSIX serial opening stays blocked by the
      unqualified DTR gate.
- [x] PI-DOMAIN-001: implement durable operations, idempotent admission and
      snapshot generation. The deferred terminal-mismatch daemon-result
      assertion is closed: an unverified terminal makes the operation
      `UNCERTAIN`.
- [x] PI-DOMAIN-002: make journal failure and priority stop fail closed. A
      refused journal write latches the machine undurable and blocks new
      motion with `JOURNAL_UNAVAILABLE`; a stop without its trusted terminal
      leaves affected work `UNCERTAIN`.
- [x] PI-DOMAIN-003: typed home and sort adapters validate advertised
      ...
      evidence.
- [x] PI-DOMAIN-004: session and configuration operations. Connect and
      recover repair an unready session; configuration is validated, journaled
      and versioned, and never reaches the controller.
