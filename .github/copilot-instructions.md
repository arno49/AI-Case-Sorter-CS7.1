...
- `platformio.ini` provides the `uno` firmware build and platform-independent `native` unit tests, including the cooperative `StepSequence` lifecycle tests. `Wire` and `SoftwareSerial` are Arduino libraries; no additional dependency manifest is checked in.
...
- The firmware is a cooperative state machine. `loop()` services serial input, proximity sensing, sorter movement/completion, cooperative sorter pre-homing jogs, feed scheduling/errors/movement/homing, nonblocking feed/AirDrop completion, timer-paced diagnostics, and motor standby. Feed movement is blocked while sorting is active; both axes home through sensor-driven states and optional offsets.
...
- Serial bytes are consumed on every cooperative loop pass. During motion, homing, sorter jog, or diagnostic pacing, one ordinary command may be pending; further complete ordinary frames receive `error:busy`. Exact `stop` frames dispatch immediately and cancel cooperative work. Runtime code has no millisecond `delay()` calls; only `delayMicroseconds()` calls that form and pace individual motor steps remain.
- Preserve these command families and response semantics:
  ...
  | `homefeeder`, `homesorter` | Starts homing and immediately acknowledges with `ok`; `homesorter` cooperatively pre-jogs before sorter homing. |
  ...
  | `test:{count}`, `sorttest:{count}` | Starts repeated hardware diagnostics over `[0, slotCount)` and emits `testing started`, followed by progress lines; sorter-only moves use a cooperative 40 ms pacing gate. |
  | `slotcount:{count}` | Sets the diagnostics-only slot count and emits `ok`, or `error:invalid slotcount` without an update. The value must fit the current sorter step geometry. |
  | `getconfig` | Emits a single JSON object containing the established runtime values. Deliberately omit `SlotCount` until Windows application tolerance for new properties is verified. |
  | `version` | Emits `FIRMWARE_VERSION` alone. |
  | `ping` | Emits the legacy response ` ok` with a leading space. |

- Runtime setters use the exact spellings implemented in `checkSerial()`: `feedspeed:`, `feedsteps:`, `feedhomingoffset:`, `sortspeed:`, `sortsteps:`, `slotcount:`, `sorthomingoffset:`, `notificationdelay:`, `slotdropdelay:`, `automotorstandbytimeout:`, `debounceTimeout:`, `debounceTime:`, `airdropenabled:`, `airdroppredelay:`, `airdroppostdelay:`, `airdropdsignalduration:`, and `cameraledlevel:`. Preserve the existing mixed-case debounce names and the legacy extra `d` in `airdropdsignalduration:` unless the desktop application is updated in lockstep. Successful setters emit `ok`.
- `SORTER_SLOT_COUNT` (default 8) and `slotCount` control only the upper-exclusive random range used by `test:` and `sorttest:`. They are independent of chute separation and do not restrict numeric, `xf:`, or `sortto:` commands. Slot count and `sortsteps:` setters cross-check the AVR 16-bit movement limit. Configure slot count manually or through advanced configuration because it is intentionally not exposed by `getconfig`.
...
- Timing settings have physical meaning: notification delay allows vibration to settle before image capture, slot-drop delay lets a case clear the sorter tube, and AirDrop pre/signal/post delays coordinate the optional solenoid. Feed completion snapshots AirDrop mode plus pre/signal/notification durations at start, then cooperatively emits signal-high, signal-low, and notify events. Keep completion busy through `done`; cancel it and force `FEED_DONE_SIGNAL` low on stop, feed overtravel, or departure from `Running`. AirDrop post-delay remains the separate slot/drop gate.
- Proximity debounce is cooperative: only an absence longer than `DebounceTimeout` arms settling, after which the sensor must remain continuously active for the full `DebouncePauseTime`. A sensor drop cancels and restarts that active interval. Initial boot with brass active does not settle. `xf:` and `FEEDSENSOR_ENABLED=false` bypass readiness without consuming an armed settle; stop and feed recovery reset active settling.
- Startup performs the cooperative sorter pre-jog before enabling either homing axis, then starts feeder and sorter homing together. `homesorter` uses the same jog but starts only sorter homing afterward. Stop and feed faults cancel the jog and must not allow deferred homing to begin.
