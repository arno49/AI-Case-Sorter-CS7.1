...
- Preserve these command families and response semantics:
  ...
  | `test:{count}`, `sorttest:{count}` | Starts repeated hardware diagnostics and emits `testing started`, followed by progress lines; sorter-only moves use a cooperative 40 ms pacing gate. |
